...
- Cloud Secrets / env vars или `memory/site.env.local` — SFTP/SSH доступ: `SSH_HOST`, `SSH_PORT`, `SSH_USER`, `SSH_PASSWORD`, `SSH_ROOT` (корень WP, где `wp-load.php`) + `PUBLIC_SITE_URL` + `EXCALIBUR_BLOG_ALLOW_PUBLISH=yes`
- Env precedence: переменные окружения перекрывают `memory/site.env.local`; legacy `FTP_*` aliases поддерживаются только для совместимости, transport всегда SFTP/SSH.
