...
- Cloud Secrets / env vars или `memory/site.env.local` — SFTP/SSH доступ + `FTP_ROOT` или `SSH_ROOT` (корень WP, где `wp-load.php`) + `PUBLIC_SITE_URL` + `EXCALIBUR_BLOG_ALLOW_PUBLISH=yes`
- Env precedence: переменные окружения перекрывают `memory/site.env.local`; для совместимости поддерживаются `FTP_HOST`, `FTP_USER`, `FTP_PASS`/`FTP_PASSWORD`, но transport всегда SFTP/SSH.
